...
while True: 
    ...
    for i in range(detections['detection_boxes'].shape[0]):
        if detections['detection_scores'][i] > 0.5:
            ...
            detected_object = image_np[top:bottom, left:right]
            class_index = detections['detection_classes'][i]
            timestamp = int(time.time())  # Menggunakan timestamp sebagai unik identifier
            image_path = f'detected_hole_{class_index}_{timestamp}.jpg'
            # cv2.imwrite(image_path, detected_object)
            latitude, longitude = get_current_location()
            print(image_path)
            print(latitude, longitude)
            # send(image_path, latitude, longitude)
            detected_objects += 1
